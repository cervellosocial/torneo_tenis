...
def generar_cuadro_enfrentamientos(df_partidos):
    ...
    for _, partido in df_partidos.iterrows():
        ...
        if partido['tie']:
            resultado += '<br>('+partido['tie']+')'
            resultado_visitante += '<br>('+resultado_contrario(partido['tie'])+')'
...
def generar_web(pagina, datos_json, template ='web_templates/torneos_grupos.html.jinja'):
    ...
    for filepath in datos_json:
        ...
        dheaders =['Posición','Jugador','Puntos','Partidos<br>Jugados','Partidos<br>Ganados','Partidos<br>Perdidos','Sets<br>Ganados','Sets<br>Perdidos','Juegos<br>Ganados','Juegos<br>Perdidos','Diferencia<br>Juegos']
    ...
    web = template.render(tablas=tablas, cuadros=cuadros, calendarios=calendarios).encode('utf-8')
